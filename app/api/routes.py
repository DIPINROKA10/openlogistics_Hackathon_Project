...
# OpenEnv Standard Endpoints (for Hugging Face Spaces validation)
openenv_router = APIRouter()

@openenv_router.post("/reset")
@limiter.limit("2/second")
async def openenv_reset(request: Request):
    ...
    try:
        global _env
        _env = OpenLogisticsEnv()
        state = _env.reset("easy_delivery")
        return {
            "status": "success",
...
@openenv_router.get("/validate")
@limiter.limit("5/second")
async def openenv_validate(request: Request):
    """OpenEnv standard: Validate endpoint."""
    try:
        env = get_env()
        if env._current_state is None:
            env.reset("easy_delivery")
        ...
        return {
            "status": "valid",
            "environment": "openlogistics",
            "version": "1.0.0",
            "state": env.state().dict() if hasattr(env.state(), 'dict') else env.state()
        }
    except Exception as e:
